...
const MyAddedJobs = () => {
  ...
  useEffect(() => {
    ...
    const url = `https://worknex-server.vercel.app/myadd?email=${user.email}`;
  ...
  return (
    <div className="grid sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5 mt-10">
      {jobs.map((jobs) => (
        <div
          key={jobs._id}
          className="card bg-base-100 w-96 shadow-sm hover:shadow-2xl  transition 
            duration-300 ease-out hover:-translate-y-2 hover:scale-[1.02] cursor-pointer"
        >
          <figure>
            <img src={jobs.coverimage} alt="Shoes" />
          </figure>
